...
PlexyCanvas *plexy_canvas_create(int width, int height,
                                 PlexyCanvasTarget target);
...
int plexy_canvas_init_with_egl(PlexyCanvas *ctx, void *egl_display,
                               void *egl_context);

void plexy_canvas_init_gl(PlexyCanvas *ctx);
...
uint32_t plexy_canvas_create_widget(PlexyCanvas *ctx, int type,
                                    uint32_t parent_id);
...
void plexy_canvas_set_placeholder(PlexyCanvas *ctx, uint32_t id,
                                  const char *text);
...
void plexy_canvas_set_translation(PlexyCanvas *ctx, uint32_t id, float x,
                                  float y);
void plexy_canvas_set_opacity(PlexyCanvas *ctx, uint32_t id, float opacity);
void plexy_canvas_set_margin(PlexyCanvas *ctx, uint32_t id, float t, float r,
                             float b, float l);
void plexy_canvas_set_padding(PlexyCanvas *ctx, uint32_t id, float t, float r,
                              float b, float l);
...
void plexy_canvas_set_drawer_open(PlexyCanvas *ctx, uint32_t id, int open,
                                  int animated);
...
void plexy_canvas_set_corner_radius(PlexyCanvas *ctx, uint32_t id,
                                    float radius);
void plexy_canvas_set_elevation(PlexyCanvas *ctx, uint32_t id, float elevation);
void plexy_canvas_set_fill_color(PlexyCanvas *ctx, uint32_t id, float r,
                                 float g, float b, float a);
void plexy_canvas_set_glass_material(PlexyCanvas *ctx, uint32_t id,
                                     float opacity);
void plexy_canvas_set_surface_material(PlexyCanvas *ctx, uint32_t id);
void plexy_canvas_set_window_shell_material(PlexyCanvas *ctx, uint32_t id);
void plexy_canvas_set_sidebar_material(PlexyCanvas *ctx, uint32_t id);
void plexy_canvas_set_content_material(PlexyCanvas *ctx, uint32_t id);
void plexy_canvas_set_header_material(PlexyCanvas *ctx, uint32_t id);
void plexy_canvas_set_menu_material(PlexyCanvas *ctx, uint32_t id);
void plexy_canvas_set_search_material(PlexyCanvas *ctx, uint32_t id);
void plexy_canvas_set_spotlight_material(PlexyCanvas *ctx, uint32_t id);
void plexy_canvas_set_border(PlexyCanvas *ctx, uint32_t id, float r, float g,
                             float b, float a, float width);
void plexy_canvas_set_text_color(PlexyCanvas *ctx, uint32_t id, float r,
                                 float g, float b, float a);
...
void plexy_canvas_quadratic_to(PlexyCanvas *ctx, float cx, float cy, float x,
                               float y);
void plexy_canvas_bezier_to(PlexyCanvas *ctx, float cp1x, float cp1y,
                            float cp2x, float cp2y, float x, float y);
void plexy_canvas_close_path(PlexyCanvas *ctx);
void plexy_canvas_set_path_stroke_color(PlexyCanvas *ctx, float r, float g,
                                        float b, float a);
...
void plexy_canvas_inject_mouse_button(PlexyCanvas *ctx, int button, int pressed,
                                      float x, float y);
void plexy_canvas_inject_mouse_scroll(PlexyCanvas *ctx, float dx, float dy,
                                      float x, float y);
void plexy_canvas_inject_key(PlexyCanvas *ctx, uint32_t keycode, int pressed,
                             uint32_t mods);
...
void plexy_canvas_set_event_callback(PlexyCanvas *ctx,
                                     PlexyCanvasEventCallback cb,
                                     void *userdata);

typedef void (*PlexyCanvasWidgetCallback)(uint32_t widget_id, void *userdata);
typedef void (*PlexyCanvasValueCallback)(uint32_t widget_id, float value,
                                         void *userdata);
typedef void (*PlexyCanvasTextCallback)(uint32_t widget_id, const char *text,
                                        void *userdata);
typedef void (*PlexyCanvasKeyCallback)(uint32_t widget_id, uint32_t keycode,
                                       uint32_t mods, void *userdata);

void plexy_canvas_on_click(PlexyCanvas *ctx, uint32_t id,
                           PlexyCanvasWidgetCallback cb, void *userdata);
void plexy_canvas_on_value_changed(PlexyCanvas *ctx, uint32_t id,
                                   PlexyCanvasValueCallback cb, void *userdata);
void plexy_canvas_on_text_changed(PlexyCanvas *ctx, uint32_t id,
                                  PlexyCanvasTextCallback cb, void *userdata);
void plexy_canvas_on_key(PlexyCanvas *ctx, uint32_t id,
                         PlexyCanvasKeyCallback cb, void *userdata);
void plexy_canvas_set_focus(PlexyCanvas *ctx, uint32_t id);

int plexy_canvas_get_widget_bounds(PlexyCanvas *ctx, uint32_t id, float *x,
                                   float *y, float *width, float *height);
int plexy_canvas_icon_index_at(PlexyCanvas *ctx, uint32_t id, float x, float y);
